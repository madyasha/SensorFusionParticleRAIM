...
#this functio returns the index of the best match of the image

def camera_prob(test_filename,test2_filename,tt,num_train,Xct,Yct,Zct,names) :
...
#return top N matches or probabilities
	...
	file_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'REAR IMAGES/BLURRED IMAGES')
	#file_path = 'C:/Users/JNM/Downloads/ParticleRAIM/REAR IMAGES/BLURRED IMAGES'
	#------------------------------------------------------------------------------------------------------------					
	rootDir = file_path		
	...
	imagesInLeaves = {}	
						# Dictionary in the format - leafID: [img1:freq, img2:freq, ..]
	doc = {}								# 							#		#
	maxDepth = 5
	avgDepth = 0
	...
	def dumpFeatures(rootDir):
		...
		for fname in fileList:
			...
			# #print (des)
			kp, des = sift.detectAndCompute(cv2.cvtColor(cv2.imread(img), cv2.COLOR_BGR2GRAY), None)
			for d in des:
				features.append(d)
			del kp, des
	...
	def findBest(scores):
		...
		sc = -1* np.asarray(scores_f)
		index = np.argsort(sc)
		return index[0], -sc[index[0]]
	...
	ind1,val1 = match(test_filename,X.tolist(),Y.tolist(),Z.tolist())
	#print ('test', test_filename)
	ind2,val2 = match(test2_filename,X.tolist(),Y.tolist(),Z.tolist())
